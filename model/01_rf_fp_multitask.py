...
# Model
def tune_rf_multitask_val(X_tr: np.ndarray, Y_tr: np.ndarray, X_va: np.ndarray, Y_va: np.ndarray, seed: int, param_grid: Dict[str, List[Any]], max_trials: int, n_jobs: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    best = {'val_rmse': float('inf'), 'params': None}
    trace = []
    for vals in combos:
        params = dict(zip(keys, vals))
        m = RandomForestRegressor(random_state=seed, n_jobs=n_jobs, **params)
        m.fit(X_tr, Y_tr)
        pred = m.predict(X_va)
        s = multi_rmse(Y_va, pred)
        trace.append({'params': params, 'val_multi_rmse_scaled': float(s)})
        if s < best['val_rmse']:
            best = {'val_rmse': float(s), 'params': params}
    if best['params'] is None:
        raise RuntimeError('RF multitask tuning failed.')
    meta = {'selection': 'val_multi_RMSE_scaled', 'max_trials': int(max_trials), 'trace': trace, 'best_val_multi_rmse_scaled': best['val_rmse']}
    return (best['params'], meta)

def fit_rf_multitask_member(X_tr: np.ndarray, Y_tr: np.ndarray, X_va: np.ndarray, X_te: np.ndarray, params: Dict[str, Any], seed: int, n_jobs: int, bagging: bool) -> Tuple[np.ndarray, np.ndarray, Any]:
    from sklearn.ensemble import RandomForestRegressor
    rng = np.random.RandomState(seed)
    if bagging:
    ...
    m = RandomForestRegressor(random_state=seed, n_jobs=n_jobs, **params)
    m.fit(X_fit, Y_fit)
    return (m.predict(X_va), m.predict(X_te), m)
...
def run_model(model_name: str, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, full_df: pd.DataFrame, out_root: Path, seed: int, n_members: int, target_transform: str, requested_targets: Optional[List[str]], use_target_scaling: bool, bagging: bool, save_models: bool) -> None:
    ...
    outdir = ensure_dir(out_root / 'rf_fp_multitask')
    ...
    run_config = {'model': 'rf_fp_multitask', 'created_utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'seed': int(seed), 'n_members': int(n_members), 'targets': targets, 'n_targets': int(n_targets), 'target_transform': str(target_transform), 'target_scaling': target_scaler.stats(targets), 'bagging': bool(bagging), 'save_models': bool(save_models), 'data_shape': {'train': list(train_df.shape), 'val': list(val_df.shape), 'test': list(test_df.shape)}, 'data_hash': {'train_sha256': sha256_df_content(train_df), 'val_sha256': sha256_df_content(val_df), 'test_sha256': sha256_df_content(test_df)}, 'split_indices': split_indices, 'versions': collect_versions(), 'seed_control': set_global_seed(seed)}
    metrics_rows: List[Dict[str, Any]] = []
    summary: Dict[str, Any] = {'model': 'rf_fp_multitask', 'n_members': int(n_members), 'targets': targets, 'notes': {'multitask': True, 'target_scaling': bool(use_target_scaling), 'selection_metric': 'validation multi-target RMSE on scaled targets'}}
    try:
        import torch
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
    except Exception:
        pass
    RF_GRID = {'n_estimators': [400, 800], 'max_depth': [None, 20, 40], 'min_samples_leaf': [1, 2, 5], 'max_features': ['sqrt', 'log2', 0.2], 'bootstrap': [True]}
    N_JOBS = -1
    TUNE_TRIALS = 30
    ...
    best_params, tune_meta = tune_rf_multitask_val(X_train, Y_train_use, X_val, Y_val_use, seed=seed, param_grid=RF_GRID, max_trials=TUNE_TRIALS, n_jobs=N_JOBS)
    for i in range(n_members):
        s = seed * 1000 + 10 + i
        pv, pt, model = fit_rf_multitask_member(X_train, Y_train_use, X_val, X_test, best_params, seed=s, n_jobs=N_JOBS, bagging=bagging)
        member_val_preds_scaled.append(pv)
        member_test_preds_scaled.append(pt)
        member_meta.append({'member': i, 'seed': s})
        if save_models:
            import joblib
    ...
    for split_name, ev, n_samples in [('val', val_eval, len(Y_val_report)), ('test', test_eval, len(Y_test_report))]:
        for tgt in targets:
            ...
            for agg_name in ['mean', 'median']:
                ...
                metrics_rows.append({'model': 'rf_fp_multitask', 'target': tgt, 'split': split_name, 'agg': agg_name, 'R2': m['R2'], 'MAE': m['MAE'], 'RMSE': m['RMSE'], 'unc_std_mean': unc['std_mean'], 'unc_iqr_mean': unc['iqr_mean'], 'n_samples': int(n_samples), 'n_members': int(n_members)})
        unc = ev['uncertainty']['__overall_macro__']
        for agg_name in ['mean', 'median']:
            m = ev['overall_macro'][agg_name]
            metrics_rows.append({'model': 'rf_fp_multitask', 'target': '__overall_macro__', 'split': split_name, 'agg': agg_name, 'R2': m['R2'], 'MAE': m['MAE'], 'RMSE': m['RMSE'], 'unc_std_mean': unc['std_mean'], 'unc_iqr_mean': unc['iqr_mean'], 'n_samples': int(n_samples), 'n_members': int(n_members)})
