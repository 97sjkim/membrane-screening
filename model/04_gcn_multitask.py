...
def pyg_models():
    ...
    class GCNNet(nn.Module):
        ...
        def __init__(self, in_dim: int, out_dim: int, hidden_dims: List[int], dropout: float):
            ...
            self.convs.append(GCNConv(in_dim, hidden_dims[0]))
            for a, b in zip(hidden_dims[:-1], hidden_dims[1:]):
...
def tune_pyg_multitask_val(arch: str, train_graphs, val_graphs, out_dim: int, seed: int, param_grid: Dict[str, List[Any]], max_trials: int, device: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for trial_idx, vals in enumerate(combos, start=1):
        ...
        pv, _, pack = train_pyg_multitask_member(arch='gcn', train_graphs=train_graphs, val_graphs=val_graphs, test_graphs=val_graphs, out_dim=out_dim, seed=seed + trial_idx - 1, params=params, device=device)
    ...
    if best['params'] is None:
        raise RuntimeError(f"{'GCN'} multitask tuning failed.")
...
def run_model(model_name: str, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, full_df: pd.DataFrame, out_root: Path, seed: int, n_members: int, target_transform: str, requested_targets: Optional[List[str]], use_target_scaling: bool, bagging: bool, save_models: bool) -> None:
    ...
    outdir = ensure_dir(out_root / 'gcn_multitask')
    ...
    run_config = {'model': 'gcn_multitask', 'created_utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'seed': int(seed), 'n_members': int(n_members), 'targets': targets, 'n_targets': int(n_targets), 'target_transform': str(target_transform), 'target_scaling': target_scaler.stats(targets), 'bagging': bool(bagging), 'save_models': bool(save_models), 'data_shape': {'train': list(train_df.shape), 'val': list(val_df.shape), 'test': list(test_df.shape)}, 'data_hash': {'train_sha256': sha256_df_content(train_df), 'val_sha256': sha256_df_content(val_df), 'test_sha256': sha256_df_content(test_df)}, 'split_indices': split_indices, 'versions': collect_versions(), 'seed_control': set_global_seed(seed)}
    metrics_rows: List[Dict[str, Any]] = []
    summary: Dict[str, Any] = {'model': 'gcn_multitask', 'n_members': int(n_members), 'targets': targets, 'notes': {'multitask': True, 'target_scaling': bool(use_target_scaling), 'selection_metric': 'validation multi-target RMSE on scaled targets'}}
    device = 'cpu'
    try:
        import torch
    ...
    if len(train_graphs) == 0 or len(val_graphs) == 0 or len(test_graphs) == 0:
        raise RuntimeError(f"[{'gcn_multitask'}] Graph generation failed for at least one split.")
    ...
    for split_name, ev, n_samples in [('val', val_eval, len(Y_val_report)), ('test', test_eval, len(Y_test_report))]:
        for tgt in targets:
            ...
            for agg_name in ['mean', 'median']:
                ...
                metrics_rows.append({'model': 'gcn_multitask', 'target': tgt, 'split': split_name, 'agg': agg_name, 'R2': m['R2'], 'MAE': m['MAE'], 'RMSE': m['RMSE'], 'unc_std_mean': unc['std_mean'], 'unc_iqr_mean': unc['iqr_mean'], 'n_samples': int(n_samples), 'n_members': int(n_members)})
        unc = ev['uncertainty']['__overall_macro__']
        for agg_name in ['mean', 'median']:
            m = ev['overall_macro'][agg_name]
            metrics_rows.append({'model': 'gcn_multitask', 'target': '__overall_macro__', 'split': split_name, 'agg': agg_name, 'R2': m['R2'], 'MAE': m['MAE'], 'RMSE': m['RMSE'], 'unc_std_mean': unc['std_mean'], 'unc_iqr_mean': unc['iqr_mean'], 'n_samples': int(n_samples), 'n_members': int(n_members)})
