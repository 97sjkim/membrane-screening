...
def pyg_models():
    ...
    class GATNet(nn.Module):
        ...
        def __init__(self, in_dim: int, out_dim: int, hidden_dims: List[int], heads: List[int], dropout: float):
            super().__init__()
            self.layers = nn.ModuleList()
            self.dropout = float(dropout)
            self.layers.append(GATConv(in_dim, hidden_dims[0], heads=heads[0], concat=True, dropout=dropout))
            for i in range(1, len(hidden_dims)):
                in_ch = hidden_dims[i - 1] * heads[i - 1]
                self.layers.append(GATConv(in_ch, hidden_dims[i], heads=heads[i], concat=True, dropout=dropout))
            self.lin = nn.Linear(hidden_dims[-1] * heads[-1], out_dim)
...
def train_pyg_multitask_member(arch: str, train_graphs, val_graphs, test_graphs, out_dim: int, seed: int, params: Dict[str, Any], device: str) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    ...
    net = ModelClass(in_dim=in_dim, out_dim=out_dim, hidden_dims=params['hidden_dims'], heads=params['heads'], dropout=params['dropout'])
    net = net.to(device)
...
def tune_pyg_multitask_val(arch: str, train_graphs, val_graphs, out_dim: int, seed: int, param_grid: Dict[str, List[Any]], max_trials: int, device: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    for trial_idx, vals in enumerate(combos, start=1):
        ...
        pv, _, pack = train_pyg_multitask_member(arch='gat', train_graphs=train_graphs, val_graphs=val_graphs, test_graphs=val_graphs, out_dim=out_dim, seed=seed + trial_idx - 1, params=params, device=device)
    ...
    if best['params'] is None:
        raise RuntimeError(f"{'GAT'} multitask tuning failed.")
...
def run_model(model_name: str, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, full_df: pd.DataFrame, out_root: Path, seed: int, n_members: int, target_transform: str, requested_targets: Optional[List[str]], use_target_scaling: bool, bagging: bool, save_models: bool) -> None:
    ...
    outdir = ensure_dir(out_root / 'gat_multitask')
    ...
    run_config = {'model': 'gat_multitask', 'created_utc': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'seed': int(seed), 'n_members': int(n_members), 'targets': targets, 'n_targets': int(n_targets), 'target_transform': str(target_transform), 'target_scaling': target_scaler.stats(targets), 'bagging': bool(bagging), 'save_models': bool(save_models), 'data_shape': {'train': list(train_df.shape), 'val': list(val_df.shape), 'test': list(test_df.shape)}, 'data_hash': {'train_sha256': sha256_df_content(train_df), 'val_sha256': sha256_df_content(val_df), 'test_sha256': sha256_df_content(test_df)}, 'split_indices': split_indices, 'versions': collect_versions(), 'seed_control': set_global_seed(seed)}
    metrics_rows: List[Dict[str, Any]] = []
    summary: Dict[str, Any] = {'model': 'gat_multitask', 'n_members': int(n_members), 'targets': targets, 'notes': {'multitask': True, 'target_scaling': bool(use_target_scaling), 'selection_metric': 'validation multi-target RMSE on scaled targets'}}
    device = 'cpu'
    try:
        import torch
    ...
    if len(train_graphs) == 0 or len(val_graphs) == 0 or len(test_graphs) == 0:
        raise RuntimeError(f"[{'gat_multitask'}] Graph generation failed for at least one split.")
    ...
    for split_name, ev, n_samples in [('val', val_eval, len(Y_val_report)), ('test', test_eval, len(Y_test_report))]:
        for tgt in targets:
            ...
            for agg_name in ['mean', 'median']:
                ...
                metrics_rows.append({'model': 'gat_multitask', 'target': tgt, 'split': split_name, 'agg': agg_name, 'R2': m['R2'], 'MAE': m['MAE'], 'RMSE': m['RMSE'], 'unc_std_mean': unc['std_mean'], 'unc_iqr_mean': unc['iqr_mean'], 'n_samples': int(n_samples), 'n_members': int(n_members)})
        unc = ev['uncertainty']['__overall_macro__']
        for agg_name in ['mean', 'median']:
            m = ev['overall_macro'][agg_name]
            metrics_rows.append({'model': 'gat_multitask', 'target': '__overall_macro__', 'split': split_name, 'agg': agg_name, 'R2': m['R2'], 'MAE': m['MAE'], 'RMSE': m['RMSE'], 'unc_std_mean': unc['std_mean'], 'unc_iqr_mean': unc['iqr_mean'], 'n_samples': int(n_samples), 'n_members': int(n_members)})
